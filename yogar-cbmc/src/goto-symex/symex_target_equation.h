...
public:
  ...
  public:
    ...
    irep_idt get_address() const {
    	return original_lhs_object.get_identifier();
    }
  ...
  typedef std::map<std::string, symbol_exprt> id_symbol_mapt;
  id_symbol_mapt id_symbol_map;
  ...
  void set_guard_symbol()
  {
		  ...
		  it++)
	  {
		  ...
		  {
			  it->guard_symbol = symbol_exprt(
					    "memory_model::guard_rw"+i2string(cnt++),
					    bool_typet());
			  exprt guard_set = equal_exprt(it->guard_symbol, it->guard);
			  constraint(guard_set, "guard", it->source);
		  }
	  }
  }

  void record_id_symbol()
  {
	  for(SSA_stepst::iterator it=SSA_steps.begin();
	      it!=SSA_steps.end(); it++)
	  {
	    if(it->is_assignment() && !it->ignore && it->rely) {
	        std::string identifier = as_string(it->ssa_lhs.get_identifier());
	   	    id_symbol_map[identifier] = it->ssa_lhs;
	    }
        if (it->is_shared_read() || it->is_shared_write())
	 	{
	 		std::string identifier = as_string(it->guard_symbol.get_identifier());
	 	    id_symbol_map[identifier] = it->guard_symbol;
        }
	  }
  }
