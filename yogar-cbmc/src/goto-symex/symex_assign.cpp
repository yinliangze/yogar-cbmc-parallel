...
{
  if(lhs.id()==ID_symbol)
    symex_assign_symbol(state, to_symbol_expr(lhs), full_lhs, rhs, guard, visibility, array_assign, struct_member);
  else if(lhs.id()==ID_index)
    symex_assign_array(state, to_index_expr(lhs), full_lhs, rhs, guard, visibility, array_assign, struct_member);
  else if(lhs.id()==ID_member)
  {
    const typet &type=ns.follow(to_member_expr(lhs).struct_op().type());
    if(type.id()==ID_struct)
      symex_assign_struct_member(state, to_member_expr(lhs), full_lhs, rhs, guard, visibility, array_assign, struct_member);
          ...
          lhs.id()==ID_complex_imag)
  {
    ...
    symex_assign_rec(state, lhs.op0(), full_lhs, new_rhs, guard, visibility, array_assign, struct_member);
  }
  else
...
//  state.rename(ssa_rhs, ns, goto_symex_statet::L2, array_assign,&struct_member);
  do_simplify(ssa_rhs);
  state.rename_unique(ssa_rhs, ns, symbol_map, goto_symex_statet::L2, array_assign, &struct_member);
  ...
  int struct_member)
{
  ...
  symex_assign_rec(
    state, lhs_array, new_full_lhs, new_rhs, guard, visibility, true,struct_member);
  #endif
}
  ...
  int struct_member)
{
  ...
  symex_assign_rec(
    state, lhs_struct, new_full_lhs, new_rhs, guard, visibility,array_assign, 1);
  #endif
}
