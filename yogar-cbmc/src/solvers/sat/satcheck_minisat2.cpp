...
template<typename T>
int satcheck_minisat2_baset<T>::nClauses()
{
...
propt::resultt satcheck_minisat2_baset<T>::prop_solve()
{
  ...
  {
    std::string msg=
      "Process " + i2string(solver->get_process_id()) + ": " + i2string(_no_variables)+" variables, "+
      i2string(solver->nClauses())+" clauses, "+i2string(solver->nLearnts())+" learnts";
    messaget::status(msg);
  }
...
//      messaget::status(msg);
    ...
    else if (!(solver->parallel_flag && refinement_buf[0] > '0'))
    {
...
satcheck_minisat_simplifiert::satcheck_minisat_simplifiert(char parallel):
  satcheck_minisat2_baset<Minisat::SimpSolver>(new Minisat::SimpSolver)
{
	solver->parallel_flag = parallel;
	...
	if (parallel != 0)
	{
	    ...
	    solver->set_process_id(process_id_buf[0]);
		process_id_buf[0] = (process_id_buf[0] + 1) % process_id_buf[1];
...
int satcheck_minisat_simplifiert::get_process_id()
{
	return solver->get_process_id();
}
