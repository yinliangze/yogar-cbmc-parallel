...
//    if (file.find("<builtin-library>") != file.npos)
//    {
//      step_to_node[it->step_nr]=sink;
//
//      continue;
...
//    }
      ...
      ) // no ++it
  {
    ...
    case goto_trace_stept::ASSERT:
    case goto_trace_stept::LOCATION:
      if(it->type!=goto_trace_stept::LOCATION || it->pc->is_goto())
      {
        xmlt edge("edge");
        edge.set_attribute("source", graphml[from].node_name);
        edge.set_attribute("target", graphml[to].node_name);

        {
          ...
          data_t.data=i2string(it->thread_nr);
  ...
  graphmlt &graphml)
{
	  ...
	  it++)
  {
	...
	const unsigned from=step_to_node[it->step_nr];
    const unsigned to = step_to_node[it->step_nr + 1];
    ...
    case goto_trace_stept::FUNCTION_CALL:
      {
		...
		xmlt &data_f=edge.new_element("data");
		data_f.set_attribute("key", "threadId");
		data_f.data=i2string(it->thread_nr);

		xmlt &data_l=edge.new_element("data");
		data_l.set_attribute("key", "startline");
		data_l.data=id2string(graphml[from].line);

    	if (id2string(it->identifier) == "c::pthread_create")
    	{
		  xmlt &data_r=edge.new_element("data");
		  data_r.set_attribute("key", "createThread");
		  data_r.data=i2string(++thr_nr);
		}
    	else
		{
		  xmlt &data_r=edge.new_element("data");
		  data_r.set_attribute("key", "enterFunction");
		  data_r.data=id2string(it->identifier).substr(3, id2string(it->identifier).size());
		}
		graphml[to].in[from].xml_node=edge;
		graphml[from].out[to].xml_node=edge;
	  }
