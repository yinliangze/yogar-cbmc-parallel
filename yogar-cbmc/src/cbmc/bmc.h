...
protected:
  ...
  void program_order_wmm_extraction(EOG& eog, prop_convt &prop_conv, bool trace_flag);
  void program_order_tso_extraction(EOG& eog, prop_convt &prop_conv, bool trace_flag);
  void graph_inition(DEOG& graph, prop_convt &prop_conv, bool trace_flag);
  ...
  void expr_simplify(exprt& expr);
  std::string expr2string(const exprt& expr);
  exprt string2expr(const std::string& str);
  exprt construct_exprt(const exprt& expr);
  void attach_shared_memory();
  void detach_shared_memory();
  exprt read_write_shared_refinement(exprt& expr);
  void str_replace(std::string& str, const std::string& src, const std::string& dst);
  std::string get_result_file_name();

private:
  int shared_refinement_id;
  int process_id;
  int process_num;
  clock_t tpbegin;
};
