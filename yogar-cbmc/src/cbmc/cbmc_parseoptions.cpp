...
void cbmc_parseoptionst::get_command_line_options(optionst &options)
{
  ...
  options.set_option("parallel",
       cmdline.isset("parallel"));
  ...
  if(cmdline.isset("share"))
      options.set_option("share", cmdline.getval("share"));
}

std::string generate_new_file_name(std::string& full_name)
{
	...
	std::string new_file_name = "yogar-tmp\/" + file_name.substr(0, file_name.size() - 1) + "s.i";
	if (access("yogar-tmp\/", 0) == -1) {
		if (!mkdir("yogar-tmp\/",0777)) {
			std::cout << "Error in create yogar-tmp directory.\n";
			new_file_name = full_name.substr(0, full_name.size() - 1) + "s.i";
		}
	}
...
int cbmc_parseoptionst::doit()
{
  ...
  std::string new_file_name = generate_new_file_name(cmdline.args[0]);
  mutex_flag = preprocess(cmdline.args[0], new_file_name);
  options.set_option("original_file", cmdline.args[0]);
  cmdline.args[0] = new_file_name;

  //
  // Print a banner
  //
  status("We use CBMC version " CBMC_VERSION " "+
  ...
  goto_functions.mutex_flag = mutex_flag;
...
//  std::string command = std::string("rm ") + new_file_name;
//  system(command.data());
  ...
  goto_functionst &goto_functions)
{
  ...
  debugx() << "Memory consumption:" << messaget::endl;
  memory_info(debugx());
  debugx() << eom;
...
void cbmc_parseoptionst::help()
{
  ...
  std::cout <<
    ...
    " --parallel                   run multiple processes in parallel\n"
    " --share SM                   share manner of parallel processes (NS, RS, CS and RCS)"
